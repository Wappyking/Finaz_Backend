...
function SignUp_public_model({ name, email, password, phone, data, uuid }) {
  return supabase
    .from("user_public")
    .insert([
      {
...
function SignUp_public_model2({ name, phone, password }) {
  return supabase
    .from("user_public")
    .insert([
      {
...
function SignUp_private_model2(payload) {
  return supabase.auth.signUp({
    // email: payload.email,
    phone: payload.phone,
    password: payload.password,
